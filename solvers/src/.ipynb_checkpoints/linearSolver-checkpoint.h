...
double Dot(VD &vec1, VD &vec2 , mpiInfo &myMPI)
{
  ...
  if (myMPI.iPE < myMPI.nPEx - 1) { iMax -= 1; } // --- TO-DO in LAB --- 
  if (myMPI.jPE < myMPI.nPEy - 1) { jMax -= 1; } // --- TO-DO in LAB --- 
  ...
  double result = 0;  // --- TO-DO in LAB --- 
  MPI_Allreduce(&sum, &result, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); // --- TO-DO in LAB ---
...
void CG(VD &Solution , mpiInfo & myMPI)
{
  ...
  rowLOOP { b_PEsum[row] = b[row]; printf("b_PEsum[%d] = %f", row, b_PEsum[row]); }  
  // --- TO-DO in LAB --- 
  myMPI.PEsum(b_PEsum); // --- TO-DO in LAB --- 
